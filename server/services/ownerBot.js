...
async function onMessage(msg) {
  ...
  if (text === '⚠️ Alerts') return sendAlerts(chatId)
...
async function sendVehicleDetail(chatId, vehicleId) {
  ...
  for (const doc of vehicle.documents) {
    ...
    const icon = daysLeft < 0 ? '🔴' : daysLeft < 30 ? '⚠️' : '✅'
...
async function sendPnlReport(chatId, period) {
  ...
  if (tollTotal > 0) text += `├ 🛣️ Tolls: ₹${tollTotal.toLocaleString('en-IN')} (${totalExpenses > 0 ? Math.round(tollTotal / totalExpenses * 100) : 0}%)\n`
  ...
  if (trips.length > 0) {
    ...
    if (sorted.length > 0) {
      ...
      if (sorted.length > 1) {
        ...
        text += `⚠️ *Lowest:* ${worstVn} — ${worstData.trips} trips\n`
      }
    }
  }
...
async function sendAlerts(chatId) {
  ...
  let text = `⚠️ *${alerts.length} Unresolved Alerts*\n\n`
  ...
  try {
    ...
    switch (event) {
      ...
      case 'trip_complete': {
        ...
        text += `🛣️ ${trip.loadingLocation} → ${trip.destination}\n`
        ...
        if (tollTotal > 0) text += `├ 🛣️ ₹${tollTotal.toLocaleString('en-IN')}\n`
        ...
        if (drivingScore && drivingScore.totalDistanceKm > 0) {
          ...
          if (totalEvents > 0) {
            text += `└ ⚠️ ${totalEvents} harsh events (${sc.harshBrakeCount} brakes, ${sc.harshAccelCount} accel, ${sc.sharpTurnCount} turns)\n`
          }
        }
        break
      }
