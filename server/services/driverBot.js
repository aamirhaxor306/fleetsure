 ...
 * 3. Expenses — fuel, toll, cash (buttons during trip)
...
const ACTIVE_TRIP_KEYBOARD = {
 keyboard: [
 [{ text: 'Diesel Bhara' }, { text: 'Toll Diya' }],
 [{ text: 'Cash Kharcha' }],
...
async function onHelp(msg) {
 ...
 `Ya likhke bhejo: \`Bhopal Indore\`\n\n`+
 `*Trip ke dauran:*\n`+
 `Diesel Bhara — fuel log karo\n`+
 `Toll Diya — toll amount daalo\n`+
...
async function onCallbackQuery(query) {
 ...
 return bot.sendMessage(chatId, 'Toll kitna laga? (₹ mein likho)')
 }
 ...
 const labels = { food: 'Khana', repair: 'Repair', parking: 'Parking', room: 'Room', other: 'Other' }
 ...
 return bot.sendMessage(chatId,
 'Theek hai, wapas ja rahe ho. Trip abhi active hai.\n\nPahunchne pe "Pahunch Gaya" dabao.',
...
async function onLocation(msg) {
 ...
 await bot.sendMessage(chatId,
 `*Live GPS Tracking* — speed aur driving behavior track karo:`,
 {
 parse_mode: 'Markdown',
 reply_markup: {
 inline_keyboard: [[{
 text: 'GPS Tracking Shuru Karo',
...
async function onMessage(msg) {
 ...
 return bot.sendMessage(chatId, 'Toll kitna laga?', {
 reply_markup: {
 inline_keyboard: [
 ...
 reply_markup: {
 inline_keyboard: [
 [{ text: 'Khana', callback_data: 'cash_type:food' }, { text: 'Repair', callback_data: 'cash_type:repair' }],
 [{ text: 'Parking', callback_data: 'cash_type:parking' }, { text: 'Room', callback_data: 'cash_type:room' }],
...
async function handleNewTrip(chatId, driver) {
 ...
 keyboard.push([{ text: 'Naya Route Likho', callback_data: 'route:custom' }])
...
async function saveTollExpense(chatId, amount) {
 ...
 setState(chatId, 'trip_active')
 return bot.sendMessage(chatId,
 `*Toll ₹${amount.toLocaleString('en-IN')} saved!*\n\n`+
 `Trip kharcha ab tak: *₹${tripTotal.toLocaleString('en-IN')}*`,
 { parse_mode: 'Markdown', reply_markup: ACTIVE_TRIP_KEYBOARD })
}
...
async function showTripSummary(chatId, tripId, driver) {
 ...
 summary += ` ${trip.loadingLocation} → ${trip.destination}\n`
 summary += ` ${hours}h ${mins}min\n\n`
 summary += `*Kharcha Summary:*\n`
 if (fuelTotal > 0) summary += `├ Diesel: ₹${fuelTotal.toLocaleString('en-IN')} (${fuelLitres}L)\n`
 if (tollTotal > 0) summary += `├ Toll: ₹${tollTotal.toLocaleString('en-IN')}\n`
 for (const c of cash) {
 ...
 summary += `├ Speed: ${sc.speedScore} | Braking: ${sc.brakingScore}\n`
 summary += `├ Accel: ${sc.accelerationScore} | Turns: ${sc.corneringScore}\n`
 ...
 summary += `└ ${sc.harshBrakeCount} harsh brakes, ${sc.harshAccelCount} accel, ${sc.sharpTurnCount} turns\n`
 }
 }
...
async function handleMyStatus(chatId, driver) {
 ...
 if (todayToll > 0) text += `Toll: ₹${todayToll.toLocaleString('en-IN')}\n`
...
async function handleLoadingSlipOCR(chatId, msg, driver) {
 ...
 text += ` ${ocrResult.originPlant || '?'} → ${ocrResult.destinationPlant || '?'}\n`
 text += `\n Fleet owner se vehicle register karwao.`
 ...
 text += ` ${ocrResult.originPlant || 'Unknown'} → ${ocrResult.destinationPlant || 'Unknown'}\n`
 text += `Slip: ${ocrResult.loadingSlipNumber || '—'}\n`
...
export async function sendEveningSummary() {
 ...
 if (tollTotal > 0) text += `Tolls: ₹${tollTotal.toLocaleString('en-IN')}\n`
